...
// Disease Configuration (9 real models)
const DISEASE_CONFIG = {
    pneumonia: {
        id: 1,
        title: "Pneumonia Detection",
        description: "Chest X-ray analysis using DenseNet121 (97.7% accuracy).",
        model: "DenseNet121",
        accuracy: "97.7%",
        processingTime: "~2s",
        ...
        icon: "🌬️",
        uploadHint: "Drag & drop chest X-rays or click to browse",
        sampleImage: "🌬️ Chest X-Ray Preview",
        preprocess: "resize=224x224, normalize",
        outputFields: ["label", "confidence"]
    },
    brain: {
        id: 2,
        title: "Brain Tumor Detection",
        description: "MRI analysis using ViT-L16 + Xception fusion.",
        model: "ViT-L16 + Xception",
        accuracy: "High",
        processingTime: "~3s",
        inputType: "MRI Scan",
        instruction: "Upload brain MRI images",
        accept: ".jpg,.jpeg,.png",
        acceptTypes: ["JPG", "PNG"],
        endpoint: "/predict/brain",
        icon: "🧠",
        uploadHint: "Drag & drop MRI scans or click to browse",
    ...
    bone: {
        ...
        description: "Fracture detection in X-ray images using CNN.",
        model: "Custom CNN",
        accuracy: "High",
        processingTime: "~2s",
        inputType: "X-Ray",
        ...
        icon: "🦴",
        uploadHint: "Drag & drop X-rays or click to browse",
    ...
    dental: {
        ...
        description: "YOLOv11 real-time detection and segmentation.",
        model: "YOLOv11",
        accuracy: "Real-time",
        processingTime: "~1s",
        inputType: "Dental X-Ray",
        instruction: "Upload dental X-ray for lesion detection",
        accept: ".jpg,.jpeg,.png",
        acceptTypes: ["JPG", "PNG"],
        endpoint: "/predict/dental",
        icon: "🦷",
        uploadHint: "Drag & drop dental images or click to browse",
        sampleImage: "🦷 Dental X-Ray Preview",
    ...
    eye: {
        ...
        description: "Fundus image analysis for retinal disease detection.",
        model: "Custom CNN",
        accuracy: "High",
        processingTime: "~2s",
        inputType: "Fundus Photo",
        instruction: "Upload fundus image",
        accept: ".jpg,.jpeg,.png",
        acceptTypes: ["JPG", "PNG"],
        endpoint: "/predict/eye",
        icon: "👁️",
        uploadHint: "Drag & drop fundus photos or click to browse",
    ...
    kidney: {
        ...
        description: "Tabular LGBM model for CKD prediction (98.75% accuracy).",
        model: "LGBM",
        accuracy: "98.75%",
        processingTime: "<1s",
        inputType: "CSV (Tabular)",
        instruction: "Upload a CSV with kidney features (first row used)",
        accept: ".csv",
        acceptTypes: ["CSV"],
        endpoint: "/predict/kidney",
        icon: "🫘",
        uploadHint: "Drag & drop kidney CSV or click to browse",
        sampleImage: "🫘 Kidney Data",
    ...
    lung: {
        ...
        description: "CT scan classification using EfficientNet-B0 (87.38% accuracy).",
        model: "EfficientNet-B0",
        accuracy: "87.38%",
        processingTime: "~3s",
        inputType: "CT Scan",
        instruction: "Upload CT scan image",
        accept: ".jpg,.jpeg,.png",
        acceptTypes: ["JPG", "PNG"],
        endpoint: "/predict/lung",
        icon: "🫁",
        uploadHint: "Drag & drop CT scans or click to browse",
    ...
    breast: {
        ...
        description: "Physics-Informed Neural Network (PINN) model.",
        model: "PINN",
        accuracy: "High",
        processingTime: "~3s",
        inputType: "Medical Image",
        instruction: "Upload ultrasound/thermal image",
        accept: ".jpg,.jpeg,.png",
        acceptTypes: ["JPG", "PNG"],
        endpoint: "/predict/breast",
        icon: "🎗️",
        uploadHint: "Drag & drop images or click to browse",
        sampleImage: "🎗️ Medical Image",
        preprocess: "resize=224x224, normalize",
        outputFields: ["label", "confidence"]
    },
    tb_covid: {
        id: 9,
        title: "TB / COVID-19 Detection",
        description: "Multi-class chest X-ray classification (Normal/TB/COVID-19).",
        model: "Custom CNN",
        accuracy: "High",
        processingTime: "~2s",
        inputType: "Chest X-Ray",
        instruction: "Upload chest X-ray for TB/COVID-19 analysis",
        accept: ".jpg,.jpeg,.png",
        acceptTypes: ["JPG", "PNG"],
        endpoint: "/predict/tb_covid",
        icon: "🦠",
        uploadHint: "Drag & drop chest X-rays or click to browse",
...
// Store last real results from backend
const LAST_RESULTS = {};
...
function showFilesPreview(files, config) {
    ...
    const filesList = files.map((file, index) => {
        ...
        const previewAction = hasImage
            ? `onclick="showImagePreview(selectedFilesData[${index}]?.data, '${file.name}', ${index + 1}, ${files.length}, ${JSON.stringify(config).replace(/"/g, '&quot;')})"`
            : '';
        const previewIcon = hasImage ? '🔍' : '📄';
        const fileTypeLabel = file.type || (file.name.endsWith('.csv') ? 'CSV' : 'FILE');
        ...
        return `
            <div class="file-preview-item" style="display: flex; align-items: center; gap: 16px; padding: 12px; background: var(--gray-50); border-radius: var(--radius-md); margin-bottom: 8px; position: relative; transition: all 0.2s;" onmouseover="this.style.background='var(--gray-100)'" onmouseout="this.style.background='var(--gray-50)'">
                ...
                <div style="font-size: 32px; ${hasImage ? 'cursor: pointer;' : ''}" ${previewAction}>${config.icon}</div>
                <div style="flex: 1; ${hasImage ? 'cursor: pointer;' : ''}" ${previewAction}>
                    ...
                    <div style="font-size: 12px; color: var(--gray-500);">${(file.size / 1024).toFixed(1)} KB · ${fileTypeLabel}</div>
                </div>
                <span class="badge success">Ready</span>
                <div class="image-preview-icon" ${previewAction} title="${hasImage ? 'Preview Image' : 'Preview not available'}">${previewIcon}</div>
...
async function callBackendAPI(diseaseKey) {
    ...
    try {
        ...
        LAST_RESULTS[diseaseKey] = mainResult;
...
function downloadCSVReport(diseaseKey) {
    ...
    selectedFilesData.forEach((file, index) => {
        const result = LAST_RESULTS[diseaseKey] || DEFAULT_RESULT;
    ...
    const avgConfidence = LAST_RESULTS[diseaseKey]?.confidence || 0;
    csvContent += `Average Confidence,${Math.round(avgConfidence)}%\n`;
            ...
            <div class="section">
                ...
                <div class="grid">
                    ...
                    <div class="info-card">
                        ...
                        <div class="value">${LAST_RESULTS[currentDisease]?.label || 'Normal findings'}</div>
                    </div>
                    <div class="info-card">
                        <div class="label">Confidence</div>
                        <div class="value">${LAST_RESULTS[currentDisease]?.confidence || 0}%</div>
                    </div>
                    <div class="info-card">
                ...
                <table>
                    ...
                    <tbody>
                        ${selectedFilesData.map((file, index) => {
                            const confidence = (LAST_RESULTS[currentDisease]?.confidence || 0);
                            const finding = LAST_RESULTS[currentDisease]?.label || 'Normal';
                            return `
                            <tr>
